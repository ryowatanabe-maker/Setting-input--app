...
st.set_page_config(page_title="FitPlus Pro v1200", layout="wide")
...
with col_s1:
    ...
    s_zone = st.selectbox("設定するゾーン", options=[""] + [z["名"] for z in st.session_state.z_list])
    if s_zone:
...
if all_scene_opts:
    ...
    if st.button("スケジュールを一括生成"):
        ...
        while curr <= datetime.combine(datetime.today(), g_end) and len(new_rows) < 10:
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
        ...
        for rep in DAY_OPTIONS:
            ...
            if not slots.empty:
                ...
                for j, (_, s) in enumerate(slots.head(6).iterrows()):
                    rows[idx_tt][23 + j*2], rows[idx_tt][24 + j*2] = f"{s['時刻'].hour}:{s['時刻'].minute:02}", s["シーン選択"].split(" [")[0]
    ...
    for i, p in enumerate(st.session_state.p_list):
        rows[i][45], rows[i][46], rows[i][47], rows[i][48], rows[i][49] = p["名"], f"{p['sd'].month}月{p['sd'].day}日", f"{p['ed'].month}月{p['ed'].day}日", f"{p['zn']}_{p['rep']}_TT", p['zn']

    # ヘッダー構築 (EOF対策: 全行必ず73列にする)
    h0 = [""] * NUM_COLS
    ...
    csv_body += ",".join(h2) + "\r\n"
    for r in rows:
        if any(r): csv_body += ",".join([str(x) for x in r]) + "\r\n"
