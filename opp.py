...
# --- 2. ゾーン登録 ---
st.header("2. ゾーン登録 🌐")
with st.form("z_form_v39", clear_on_submit=True):
...
with st.form("g_form_v39", clear_on_submit=True):
...
with st.form("tt_v39"):
    ...
    for i in range(st.session_state.tt_slots_count):
        ...
        tv, sv = c_t.text_input(f"時間{i+1}", value=dt, key=f"t39_{i}"), c_s.selectbox(f"シーン{i+1}", v_scenes, index=v_scenes.index(ds) if ds in v_scenes else 0, key=f"s39_{i}")
...
if st.session_state.tt_list:
    st.subheader("現在のタイムテーブル一覧（行をクリックして詳細表示） 📋")
    tt_summary_df = pd.DataFrame([{"タイムテーブル名": x["tt_name"], "ゾーン": x["zone"], "登録数": len(x["slots"])} for x in st.session_state.tt_list])
    
    # on_select を使った選択可能なデータフレーム
    event = st.dataframe(
        tt_summary_df,
    ...
    if len(event.selection.rows) > 0:
        ...
        target_tt = st.session_state.tt_list[selected_row_index]
        
        with st.expander(f"【詳細表示】{target_tt['tt_name']} のスケジュール内容", expanded=True):
            st.info(f"ゾーン: {target_tt['zone']} / 日出シーン: {target_tt['sun_start']} / 日没シーン: {target_tt['sun_end']}")
            st.table(pd.DataFrame(target_tt['slots']))
            
            if st.button("このタイムテーブルを削除 🗑️", key="del_tt_btn"):
                st.session_state.tt_list.pop(selected_row_index)
                st.rerun()
    else:
        st.caption("詳細を確認するには、上の表の行をクリックしてください。")
...
with col_apply1:
    ...
    with st.form("ts_v37"):
        ...
        if mode == "毎日(daily)":
            d_tt = st.selectbox("タイムテーブル名", v_tt_names)
