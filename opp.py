...
st.set_page_config(page_title="FitPlus Pro v1600", layout="wide")
...
with c_s1:
    s_name = st.text_input("作成するシーン名 (例: 朝)")
    s_zone = st.selectbox("設定対象ゾーン", options=[""] + vz)
    
    if s_zone:
        target_gs = [g for g in st.session_state.g_list if g["ゾ"] == s_zone]
        scene_tmp = []
        for g in target_gs:
            with st.expander(f"⚙️ {g['名']} の設定 ({g['型']})"):
                ...
                if "Synca" in g['型']:
                    ...
                    if os.path.exists("synca_palette.png"):
                        st.image("synca_palette.png", caption="演出パレット(X:1-11, Y:1-11)", width=300)
                    
                    mode = st.radio("設定モード", ["パレット(演出)", "調色(色温度)"], key=f"m_{g['名']}", horizontal=True)
                    if mode == "パレット(演出)":
                        cx, cy = st.columns(2)
                        ex = cx.slider("演出X", 1, 11, 6, key=f"x_{g['名']}")
                        ey = cy.slider("演出Y", 1, 11, 6, key=f"y_{g['名']}")
                    else:
                        kel = st.text_input("色温度(K)", "4000", key=f"k_s_{g['名']}")
                elif g['型'] == "調光調色":
...
if all_scene_opts:
    st.subheader("⏰ スケジュール自動生成・編集")
    c1, c2, c3 = st.columns(3)
    g_start, g_end, g_int = c1.time_input("開始", value=time(10, 0)), c2.time_input("終了", value=time(21, 0)), c3.number_input("間隔(分)", 1, 120, 8)
    sa, sb, sr = st.selectbox("シーンA", all_scene_opts), st.selectbox("シーンB(交互)", ["なし"]+all_scene_opts), st.selectbox("適用曜日", DAY_OPTIONS)
    
    if st.button("スケジュールを一括生成"):
        new_r = []
        curr, idx = datetime.combine(datetime.today(), g_start), 0
        while curr <= datetime.combine(datetime.today(), g_end) and len(new_r) < 10:
            new_r.append({"時刻": curr.time(), "シーン選択": (sa if idx%2==0 or sb=="なし" else sb), "繰り返し": sr})
    ...
    st.session_state.t_df = st.data_editor(
        ...
        column_config={
            ...
            "シーン選択": st.column_config.SelectboxColumn("使用シーン", options=all_scene_opts, required=True),
    ...
    with st.form("p_form", clear_on_submit=True):
        pn, p_target_z = st.text_input("名称"), st.selectbox("対象ゾーン", options=vz)
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    ...
    for i, r in enumerate(st.session_state.s_list):
        ...
        rows[i][9], rows[i][10], rows[i][11], rows[i][12], rows[i][13], rows[i][15], rows[i][16] = r["sn"], "", r["dim"], color, "static", r["zn"], r["gn"]
    
    # タイムテーブル (ID空欄)
    idx_tt = 0
    for z_name in [z["名"] for z in st.session_state.z_list]:
        for rep in DAY_OPTIONS:
            ...
            if not slots.empty:
                ...
                rows[idx_tt][18], rows[idx_tt][19], rows[idx_tt][20] = tt_n, "", z_name
    ...
    def join_r(a): return ",".join([str(x) for x in a]) + "\r\n"
    csv_b = join_r(h0) + "," * (NUM_COLS - 1) + "\r\n" + join_r(h2)
    for r in rows:
        if any(r): csv_b += join_r(r)

    tar_b = io.BytesIO()
    with tarfile.open(fileobj=tar_b, mode="w", format=tarfile.USTAR_FORMAT) as tar:
        csv_bytes = csv_b.encode("utf-8-sig")
    ...
    st.download_button(f"📥 {shop_name}.tar を保存", tar_b.getvalue(), f"{shop_name}.tar")
