...
with c_view:
    ...
    for i, z in enumerate(st.session_state.z_list):
        cols = st.columns([4, 1]); cols[0].info(f"ゾーン: {z['名']}"); 
...
# --- 2. シーン作成 ---
st.header("2. シーン作成")
c_s_main, c_s_list = st.columns([1, 1])
...
with c_s_list:
    ...
    if not st.session_state.s_list: st.write("なし")
        ...
        for (sn, zn), group in h_df.groupby(['sn', 'zn']):
            sc1, sc2 = st.columns([4, 1]); sc1.markdown(f"**{sn}** ({zn})");
            if sc2.button("削除", key=f"del_sc_{sn}_{zn}"):
...
if all_scene_names:
    with st.expander("⏰ スケジュール自動生成"):
        ...
        if st.button("生成"):
            ...
            while dt <= datetime.combine(datetime.today(), gen_end) and len(new_rows) < 10:
                target = sa if (count % 2 == 0 or sb == "なし") else sb
                new_rows.append({"時刻": dt.time(), "シーン選択": target, "繰り返し": sr})
                dt += timedelta(minutes=gen_int); count += 1
    ...
    st.session_state.t_df = st.data_editor(
        ...
        column_config={
            ...
            "シーン選択": st.column_config.SelectboxColumn("シーン選択", options=all_scene_names, required=True),
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    ...
    idx_tt = 0
    for z_name in vz:
        for rep_m in DAY_OPTIONS:
            slots = st.session_state.t_df[st.session_state.t_df["シーン選択"].str.contains(f"\[{z_name}\]", na=False) & (st.session_state.t_df["繰り返し"] == rep_m)].copy()
            if not slots.empty:
    ...
    csv_str = join_r(h0) + ("," * (NUM_COLS - 1)) + "\r\n" + join_r(h2)
    for r in rows:
        if any(r): csv_str += join_r(r)
