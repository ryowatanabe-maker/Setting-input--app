...
st.set_page_config(page_title="FitPlus Pro v9500", layout="wide")
...
st.title("FitPlus ⚙️ 統合設定ツール (赤池店インデックス再現)")
...
with st.container(border=True):
    ...
    new_scene = c_add.selectbox("順序に追加", options=current_opts)
    if c_add.button("＋ 追加"):
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    ...
    # Timetable (R列(17)開始・W列(22)時刻スロット)
    idx_tt = 0
    for z_name in vz:
        for rep in DAY_OPTIONS:
            ...
            if not slots.empty:
                ...
                # 赤池店ルール: R(17)名前, S(18)空, T(19)ゾーン
                rows[idx_tt][17], rows[idx_tt][18], rows[idx_tt][19] = sn_main, "", z_name
                for j, (_, s) in enumerate(slots.iterrows()):
                    col_base = 22 + j*2 # W列(22)から開始
                ...
                rep_idx = 0 if rep == "毎日" else DAY_OPTIONS.index(rep)
                rows[idx_tt][IDX_ZONE_TS + 1 + rep_idx] = sn_main
                idx_tt += 1
    # 特異日 (Index 207)
