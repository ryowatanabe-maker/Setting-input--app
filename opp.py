...
st.set_page_config(page_title="FitPlus Pro v350", layout="wide")
...
if 't_slots' not in st.session_state: st.session_state.t_slots = pd.DataFrame(columns=["時刻", "シーン選択", "繰り返し"])
...
if st.sidebar.button("全データをリセット"):
    ...
    st.session_state.t_slots = pd.DataFrame(columns=["時刻", "シーン選択", "繰り返し"])
...
if not all_scene_opts:
    ...
    st.subheader("⏰ スケジュール (シーン主導・繰り返し)")
    if st.session_state.t_slots.empty:
    ...
    if st.button("スケジュール保存"): st.session_state.t_slots = tt_editor; st.success("保存完了")
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    ...
    # SyncaのパレットをExcelで日付化させないために quoted 文字列として出力
    csv_buf = io.StringIO(); final_df.to_csv(csv_buf, index=False, header=False, lineterminator='\r\n', quoting=1)
