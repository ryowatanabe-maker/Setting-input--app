...
st.set_page_config(page_title="FitPlus Pro v150", layout="wide")
...
with c1:
    ...
    with st.form("z_form", clear_on_submit=True):
        zn = st.text_input("ゾーン名 (最大32文字) [cite: 32]", max_chars=32)
        zf = st.number_input("フェード時間(秒) [cite: 242]", 0, 3600, 0)
...
with c2:
    ...
    with st.form("g_form", clear_on_submit=True):
        gn = st.text_input("グループ名 [cite: 32]", max_chars=32)
        gt = st.selectbox("タイプ [cite: 28]", list(GROUP_TYPES.keys()))
...
if t_zone:
    ...
    if scenes:
        ...
        if st.button("⏰ スケジュールを一括自動生成", use_container_width=True):
            ...
            while curr <= end and len(new_slots) < 100: [cite: 449]
    ...
    if f"tt_{t_zone}" not in st.session_state:
        st.session_state[f"tt_{t_zone}"] = [{"時刻": "00:00", "シーン": scenes[0] if scenes else ""}] [cite: 445]
