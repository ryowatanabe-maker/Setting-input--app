...
if st.session_state.s_list:
    ...
    disp_df.index += 1
    ev_s = st.dataframe(disp_df, use_container_width=True, on_select="rerun", selection_mode="single-row", key="s_select_v50")
...
with st.container(border=True):
    ...
    new_scene_name = col_sn1.text_input("シーン名", value=def_s_name, key="sc_input_v50")
    ...
    if sel_zone_for_scene:
        ...
        for g in target_groups:
            ...
            if "Synca" in gt:
                with c_synca:
                    # パレットの図を折りたたみの中に表示
                    with st.expander("🎨 Syncaカラーパレット図を表示"):
                        # パレット画像を読み込む（ファイル名を合わせてください）
...
if st.session_state.tt_list:
    st.subheader("タイムテーブル履歴")
    tt_sum_df = pd.DataFrame([{"案": x["tt_name"], "ゾーン": x["zone"], "数": len(x["slots"])} for x in st.session_state.tt_list])
    ev = st.dataframe(tt_sum_df, use_container_width=True, on_select="rerun", selection_mode="single-row", key="tt_sel_v50")
    if len(ev.selection.rows) > 0:
        sel = st.session_state.tt_list[ev.selection.rows[0]]
        with st.expander(f"{sel['tt_name']} の中身", expanded=True):
...
with col_a1:
    ...
    with st.form("ts_v50"):
        ...
        target_tt = st.selectbox("適用する案", v_tt_names)
        checked_days = []
        if mode == "曜日を指定して登録":
...
if st.session_state.ts_list or st.session_state.period_list:
    ...
    if st.session_state.ts_list: st.table(pd.DataFrame([{"ゾーン": x["zone"], "毎日": x["config"]["daily"], "曜日別": ", ".join([f"{jp}:{x['config'][en]}" for en, jp in {"mon":"月", "tue":"火", "wed":"水", "thu":"木", "fri":"金", "sat":"土", "sun":"日"}.items() if x['config'][en]])} for x in st.session_state.ts_list]))
