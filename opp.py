...
if 'tt_slots_count' not in st.session_state:
    st.session_state.tt_slots_count = 1
if 'auto_scene_count' not in st.session_state:
    st.session_state.auto_scene_count = 2

# --- 2. 機器・店舗設定 ---
st.header("0. 機器と店舗の設定")
col_opt1, col_opt2 = st.columns(2)
with col_opt1:
    gw_type = st.radio("機器選択", ["BBR4HG (72列形式)", "メインゲートウェイ (65列形式)"], horizontal=True, key="gw_select_main")
shop_name = col_opt2.text_input("店舗名", value="店舗A", key="shop_name_input")
st.divider()
...
with st.container(border=True):
    ...
    z_n = cz1.text_input("ゾーン名 (例: 店内, 店外)", key="zone_name_input")
    z_f = cz2.number_input("フェード秒", 0, 60, 0, key="zone_fade_input")
    if cz3.button("ゾーンを追加 ➕", use_container_width=True, key="btn_add_zone"):
        if z_n:
            st.session_state.z_list.append({"名": z_n, "秒": z_f})
            st.rerun()

if st.session_state.z_list:
    st.subheader("現在のゾーンリスト")
...
with st.container(border=True):
    ...
    gn = cg1.text_input("グループ名", key="group_name_input")
    gt = cg2.selectbox("タイプ", list(GROUP_TYPE_MAP.keys()), key="group_type_select")
    gz = cg3.selectbox("所属ゾーン", options=v_zones, key="group_zone_select")
    if cg4.button("グループ追加 ➕", use_container_width=True, key="btn_add_group"):
        if gn and gz:
            st.session_state.g_list.append({"名": gn, "型": gt, "ゾ": gz})
            st.rerun()

if st.session_state.g_list:
    st.subheader("現在のグループリスト")
...
with st.container(border=True):
    ...
    s_name_in = csn.text_input("シーン名 (例: 日中)", key="scene_name_input")
    s_zone_in = csz.selectbox("設定対象ゾーン", options=v_zones, key="scene_zone_select")
    ...
    if s_zone_in:
        ...
        for g in target_gs:
            ...
            dim = c1.number_input("調光%", 0, 100, 100, key=f"d_val_{g['名']}")
            kel = c2.text_input("色温度", "3500", key=f"k_val_{g['名']}") if g['型'] != "調光" else ""
            syn = ""
            if "Synca" in g['型']:
                with c3:
                    cs1, cs2 = st.columns(2)
                    r_val = cs1.selectbox("行", ["-"] + list(range(1, 12)), key=f"r_val_{g['名']}")
                    c_val = cs2.selectbox("列", ["-"] + list(range(1, 12)), key=f"c_val_{g['名']}")
...
with tab1:
    ...
    with st.form("tt_form_main"):
        tt_name = st.text_input("案の名前 (例: 通常, セール時)", "通常")
        tt_zone = st.selectbox("対象ゾーン", v_zones, key="tt_zone_sel")
        sun_s = st.selectbox("日出シーン", v_s_names, key="sun_s_sel")
        sun_e = st.selectbox("日没シーン", v_s_names, key="sun_e_sel")
        slots = []
        for i in range(4):
            c1, c2 = st.columns(2)
            t_box = c1.text_input(f"時刻{i+1}", "09:00" if i==0 else "", placeholder="HH:MM", key=f"time_box_{i}")
            s_box = c2.selectbox(f"シーン{i+1}", v_s_names, key=f"scene_box_{i}")
            if t_box and s_box: slots.append({"t": t_box, "s": s_box})
...
if st.session_state.tt_list:
    for i, t in enumerate(st.session_state.tt_list):
        cl, cr = st.columns([5, 1])
        cl.write(f"⏳ {t['name']} (ゾーン: {t['zone']})")
        if cr.button("削除", key=f"del_tt_list_{i}"):
            st.session_state.tt_list.pop(i); st.rerun()

with tab2:
    v_tt = [""] + [t["name"] for t in st.session_state.tt_list]
    with st.form("ts_form_main"):
        target_tt = st.selectbox("適用する案", v_tt, key="target_tt_sel")
        if st.form_submit_button("毎日(daily)として適用"):
            if target_tt:
                zone_val = next(t["zone"] for t in st.session_state.tt_list if t["name"] == target_tt)
                st.session_state.ts_list = [x for x in st.session_state.ts_list if x["zone"] != zone_val]
                st.session_state.ts_list.append({"zone": zone_val, "daily": target_tt})
                st.rerun()

with tab3:
    with st.form("p_form_main"):
        p_name = st.text_input("特異日名 (例: 正月)", key="p_name_in")
        p_tt = st.selectbox("使用する案", v_tt, key="p_tt_sel")
        p_start = st.text_input("開始(MM/DD)", "01/01", key="p_start_in")
        p_end = st.text_input("終了(MM/DD)", "01/03", key="p_end_in")
        if st.form_submit_button("特異日として保存"):
            if p_tt:
                zone_val = next(t["zone"] for t in st.session_state.tt_list if t["name"] == p_tt)
                st.session_state.period_list.append({"name": p_name, "zone": zone_val, "tt": p_tt, "start": p_start, "end": p_end})
                st.rerun()

st.divider()

# --- 7. CSV作成ロジック (エラー対策版) ---
if st.button("CSV作成・ダウンロード 💾", type="primary", key="btn_download_final"):
    # ヘッダー構築
    ...
    st.download_button("📥 完成したCSVをダウンロード", buf.getvalue(), f"{shop_name}_FitPlus.csv", "text/csv", key="download_btn_final_exec")
