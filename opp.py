...
st.title("FitPlus ⚙️ 統合設定ツール (成功モデル完全模倣版)")
shop_name_input = st.sidebar.text_input("店舗名", "FitPlus_Project")

# --- UI (ゾーン/グループ/シーン/スケジュール) ---
...
if s_zone_in:
    ...
    for g in target_gs:
        with st.expander(f"💡 {g['名']} 設定"):
            ...
            ex, ey, kel = "", "", "4000"
            if "Synca" in g['型']:
                mode = st.radio("設定方法", ["パレット(6/6等)", "調色"], key=f"m_{g['名']}", horizontal=True)
                if mode == "パレット(6/6等)":
                    cx, cy = st.columns(2)
...
if all_scene_opts:
    ...
    if st.button("一括生成"):
        ...
        while dt <= datetime.combine(datetime.today(), g_end) and len(new_r) < 10:
    ...
    st.session_state.t_df = st.data_editor(st.session_state.t_df, column_config={"シーン選択": st.column_config.SelectboxColumn("シーン選択", options=all_scene_opts), "繰り返し": st.column_config.SelectboxColumn("曜日", options=DAY_OPTIONS)}, num_rows="dynamic", use_container_width=True)

# --- 3. 出力処理 (全ID/Perform空欄 & 全行73列カンマ72個固定 & 1000行パディング) ---
st.divider()
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    # 成功ファイルと同様に1000行分の空データを作成
