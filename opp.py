        ...
        for (sn, zn), group in h_df.groupby(['sn', 'zn']):
            with st.container(border=True):
                ...
                for _, r in group.iterrows():
                    color = f"{r['ex']}/{r['ey']}" if r['ex'] else f"{r['kel']}K"
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    ...
    for i, r in enumerate(st.session_state.s_list):
        # Syncaの数値をExcelの日付変換から守るため / を使用しつつ文字列処理
    ...
    for z_name in vz:
        for rep in DAY_OPTIONS:
            slots = [s for s in st.session_state.t_slots if f"[{z_name}]" in s["シーン選択"] and s["繰り返し"] == rep]
            if slots:
