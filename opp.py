...
st.set_page_config(page_title="FitPlus Pro v240", layout="wide")
...
with c1:
    ...
    for i, z in enumerate(st.session_state.z_list):
        cols = st.columns([4, 1])
        # 履歴にフェード時間を表示
        cols[0].info(f"ゾーン: {z['名']} / フェード: {z['秒']}秒")
...
with c2:
    ...
    for i, g in enumerate(st.session_state.g_list):
        cols = st.columns([4, 1])
        # 履歴にグループタイプを表示
        cols[0].success(f"グループ: {g['名']} / ゾーン: {g['ゾ']} / タイプ: {g['型']}")
...
with c_edit:
    ...
    s_name = st.text_input("シーン名 (例: 営業時間外)")
        ...
        for (sn, zn), group in hist_df.groupby(['sn', 'zn']):
            with st.container(border=True):
                ...
                for _, r in group.iterrows():
                    color_info = f"X/Y:{r['ex']}/{r['ey']}" if r['ex'] else f"{r['kel']}K"
                    details.append(f"- {r['gn']}: {r['dim']}% / {color_info}")
                st.write("\n".join(details))
...
if not all_scene_options:
    ...
    st.caption("シーンを選んで時刻を指定してください。ゾーンごとに自動的に振り分けられます。")
    ...
    for z_name in vz:
        ...
        if z_scenes:
            c1, c2 = st.columns(2)
            s_st = c1.selectbox(f"日の出シーン [{z_name}]", options=[""] + z_scenes, key=f"sun_st_{z_name}")
            s_en = c2.selectbox(f"日の入りシーン [{z_name}]", options=[""] + z_scenes, key=f"sun_en_{z_name}")
