...
st.set_page_config(page_title="FitPlus Pro v140", layout="wide")
...
if s_zone:
    ...
    for g in target_gs:
        with st.expander(f"■ {g['名']} ({g['型']})", expanded=True):
            ...
            ex, ey, kel = "", "", "4000"
            if "Synca" in g['型']:
                mode = st.radio(f"設定方式 ({g['名']})", ["パレット(1-11)", "色温度(K)"], horizontal=True)
                if mode == "パレット(1-11)":
                    cx, cy = st.columns(2)
                    ex, ey = cx.slider("演出X", 1, 11, 6), cy.slider("演出Y", 1, 11, 6)
...
# --- 5. タイムテーブル設定 (24時間網羅ルール) ---
st.header("3. タイムテーブル設定")
t_zone = st.selectbox("タイムテーブルを設定するゾーン", options=[""] + vz, key="t_sz")

if t_zone:
    scenes = sorted(list(set([s["sn"] for s in st.session_state.s_list if s["zn"] == t_zone])))
    st.subheader("⏰ タイムテーブル編集")
    st.caption("※24時間スケジュールを組むため、最初の時刻は 00:00 である必要があります。")
    
    # 自動生成
    c1, c2, c3 = st.columns(3)
    start_t = c1.time_input("自動生成 開始", value=datetime.strptime("00:00", "%H:%M").time())
    end_t = c2.time_input("自動生成 終了", value=datetime.strptime("23:59", "%H:%M").time())
    interval = c3.number_input("間隔(分)", 1, 120, 8)
    
    if st.button("⏰ スケジュールを一括自動生成"):
        if not scenes: st.error("シーンを登録してください")
        else:
            new_slots = []
...
if st.button("📦 ゲートウェイ用 .tar を生成", type="primary", use_container_width=True):
    ...
    for i, r in enumerate(st.session_state.s_list):
        ...
        # index 13: perform, 14: fresh-key, 15: zone, 16: group
        df_data.iloc[i, 9:17] = [r["sn"], "", r["dim"], color, "static", "", r["zn"], r["gn"]]
        
    # Timetable (18-33)
    for i, t in enumerate(st.session_state.t_list):
        # 18: TT名, 19: ID, 20: ゾーン, 21-22: Sun設定, 23-: Time/Scene
        df_data.iloc[i, 18:21] = [t["tn"], T_ID_BASE + i, t["zn"]]
        for j, slot in enumerate(t["slots"]):
            if 23 + j*2 + 1 < 34: # タイムテーブル情報エリア内
